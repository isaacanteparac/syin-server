...
public class Perfil {
	...
	private Perfil fromHashMap ( HashMap<String,Object> perfil) {
		...
		this.setCtlg_estados((Ctlg_estados)perfil.get("ctlg_estados"));
	...
	private Perfil delete() throws SQLException{
		DB db = new DB();
		PreparedStatement pst = db.prepare("Delete FROM perfiles WHERE id=?");
